...
def geometry(d):
    ...
    using the currently selected grid density (GRID). Full frame, centred."""
    w, h = d["w"], d["h"]
    cx, cy = w/2.0, h/2.0
...
function setMethod(m){method=m;setTab();if(G)draw();renderList();loadSummary();updateStale();}
function updateStale(){let el=document.getElementById('staleWarn');if(!el)return;
 ...
 el.textContent=msg;}
async function select(name){cur=name;G=await(await fetch('/api/geom/'+name)).json();
...
cv.onclick=e=>{const r=cv.getBoundingClientRect();const x=(e.clientX-r.left)/scale,y=(e.clientY-r.top)/scale;
